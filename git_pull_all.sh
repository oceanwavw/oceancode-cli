#!/bin/bash
# Pull main branch of all OceanWave v1 repositories from specified remote
# Usage: git_pull_all.sh [remote_name]
...
if [ -n "$1" ]; then
    ...
    read -p "Enter remote name to pull from (e.g., origin): " REMOTE
fi
...
for r in $REPOS; do
    if [ -d "$ROOT/$r/.git" ]; then
        ...
        if ! git remote get-url "$REMOTE" >/dev/null 2>&1; then
            ...
            # Checkout main branch and pull from specified remote
