@echo off
REM Pull main branch of all OceanWave v1 repositories from specified remote
REM Usage: git_pull_all.bat [remote_name]

setlocal enabledelayedexpansion

echo.
echo ========================================
echo Pull All OceanWave v1 Repositories
echo ========================================
echo.
...
if not "%~1"=="" (
    ...
    set /p "REMOTE=Enter remote name to pull from (e.g., origin): "
)
...
for %%r in (%REPOS%) do (
    if exist "%~dp0..\%%r\.git" (
        echo.
        echo ======================================
        echo Pulling %%r from %REMOTE%/main
        echo ======================================
        ...
        if !ERRORLEVEL! NEQ 0 (
            ...
            REM Checkout main branch and pull from specified remote
            git checkout main >nul 2>&1
            git pull %REMOTE% main
            if !ERRORLEVEL! EQU 0 (
                echo SUCCESS: Pulled %%r from %REMOTE%/main
            ) else (
                echo ERROR: Failed to pull %%r from %REMOTE%/main
            )
        )
    ) else (
